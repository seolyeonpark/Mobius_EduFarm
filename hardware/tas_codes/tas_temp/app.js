...
var dist_0 = 0;
var dist_1 = 0;
var temp_update = false;

setInterval(() => {
    temp.read(11, 4, function(err, temperature, humidity) {
        if (!err) {
            dist_1 = temperature;

            if (Math.abs(dist_1 - dist_0) > .2)
                temp_update = true;
            else
                temp_update = false;
                ...
                if (tas_state=='upload') {
                    for(var i = 0; i < upload_arr.length; i++) {
                        if(upload_arr[i].id != "temp") {
                            var cin = {ctname: upload_arr[i].ctname, con: temperature.toFixed(1) + "," + humidity.toFixed(1)};
